...
// Global process-level crash guards to prevent automatic shutdowns
process.on("unhandledRejection", (reason, promise) => {
  logger.error({ promise, reason }, "TRADING ENGINE CRITICAL: Unhandled Promise Rejection detected");
});

process.on("uncaughtException", (err) => {
  logger.error({ err }, "TRADING ENGINE CRITICAL: Uncaught Exception detected");
});
